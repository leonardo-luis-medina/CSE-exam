    ...
    <div>
      ...
      <div className="max-w-2xl mx-auto p-8">
        <div className="soft-panel rounded-2xl p-8">
          <h1 className="text-2xl font-bold mb-2 text-gray-900">Set Up Your Exam</h1>
          ...
          <div className="mb-6 year-panel rounded-lg">
            <button
            ...
            >
              ...
              <span className="text-blue-700">{yearExpanded ? "Hide options ▲" : "Change ▼"}</span>
            </button>

            {yearExpanded && (
              <div className="px-4 pb-4 flex flex-wrap gap-2 border-t border-blue-100 pt-3">
                <button
                  onClick={() => setSelectedYearTags([])}
                  className={`px-3 py-1.5 rounded-full text-sm ${
                    selectedYearTags.length === 0 ? "pill-select-active" : "pill-select"
                  }`}
                >
                  All Years
                </button>
                {visibleYears.map((y) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-sm ${
                      selectedYearTags.includes(y.year.toString()) ? "pill-select-active" : "pill-select"
                    }`}
                  >
                    {y.year}
                  </button>
                ...
                {reviewerAllowed && (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-sm ${
                      selectedYearTags.includes("reviewer") ? "pill-select-active" : "pill-select"
                    }`}
                  >
                    Reviewer Only
                  </button>
          ...
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex gap-3 text-sm">
              <button onClick={selectAll} className="text-blue-700 hover:underline">
                Select All
              </button>
